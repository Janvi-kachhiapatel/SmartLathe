...
class AIRecommendationService {
  static List<String> generate({
    ...
    // RPM Analysis
    if (actualRPM < recommendedRPM * 0.95) {
      tips.add("🟡 Increase spindle RPM to improve cutting efficiency.");
    } else if (actualRPM > recommendedRPM * 1.05) {
      tips.add("🟡 Reduce spindle RPM to prevent premature tool wear.");
    } else {
      tips.add("🟢 Spindle RPM is within the recommended range.");
    }

    // Feed Analysis
    if (actualFeed > recommendedFeed * 1.10) {
      tips.add("🟠 Feed rate is higher than recommended. Check tool load.");
    } else {
      tips.add("🟢 Feed rate is optimal.");
    }

    // Temperature Analysis
    if (temperature > 55) {
      tips.add("🔴 High temperature detected. Verify coolant flow immediately.");
    } else {
      tips.add("🟢 Temperature is normal.");
    }
    ...
    // Vibration Analysis
    if (vibration > 2.5) {
      tips.add("🔴 Excessive vibration detected. Check workpiece or tool clamping.");
    } else {
      tips.add("🟢 Structural vibration is within safe limits.");
    }

    // Power Analysis
    if (power > 4.5) {
      tips.add("🟠 Power consumption is abnormally high. Check for tool dullness.");
    }

    // Fallback if everything is running perfectly
    if (tips.isEmpty) {
      tips.add("🟢 Machine operating within optimal conditions.");
    }
